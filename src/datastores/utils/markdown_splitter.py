...
class MarkdownSplitter:
    def __init__(self, max_length, text_chunk_overlap=200):
        self.max_length = max_length
        self.text_chunk_overlap = text_chunk_overlap

    def split(self, path:Union[Path, str, None] = None, data:Union[str, None] = None)->List[str]:
        """Splits a markdown document into manageable chunks."""
        if path:
            if isinstance(path, Path):
                with path.open('r') as file:
                    self.markdown_str = file.read()
            else:
                with open(path, 'r') as file:
                    self.markdown_str = file.read()
        elif data:
            self.markdown_str = data
        else:
    ...
    def _split_chunk(self, chunk:Union[MarkdownTextChunks, MarkdownTableChunks])->List[Union[MarkdownTextChunks, MarkdownTableChunks]]:
        ...
        if isinstance(chunk, MarkdownTextChunks):
            if (self.max_length - chunk.heading_length < 0):
                raise ValueError('Got heading larger then specified chunk size.')

            text_splitter = RecursiveCharacterTextSplitter(
                separators=["\n\n", ".", "?", "\n", " "],
                chunk_size=self.max_length - chunk.heading_length,
    ...
    def _merge_text_chunks(self, chunks:List[Union[MarkdownTextChunks, MarkdownTableChunks]])->List[Union[MarkdownTextChunks, MarkdownTableChunks]]:
        ...
        for chunk in chunks:
            if isinstance(current_chunk, MarkdownTextChunks) and isinstance(chunk, MarkdownTextChunks):
                ...
                if combined_chunk.length <= self.max_length:
    ...
    def _final_merge(self, chunks):
        ...
        for chunk in chunks:
            ...
            if len(current_chunk) + len(chunk_md) < self.max_length:
