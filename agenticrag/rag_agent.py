...
from agenticrag.types import BaseMessage, LLMTool, RAGAgentResponse
...
from agenticrag.utils.helpers import FinalAnswerArgsSchema, extract_json_blocks, format_datasets, format_tool_metadata
...
class RAGAgent(RAGAgentLoaderMixin):
    ...
    def __init__(
        self,
        model: Union[LLMClient, str] = "openai/gpt-4o",
        ...
        Args:
            llm (LLMClient | str): The language model used for decision-making and tool selection.
        ...
        """
        self.llm = LLMClient(model) if isinstance(model, str) else model
        ...
        if not tasks:
            tasks = [QuestionAnsweringTask(llm=self.llm)]
        else:
            seen_type = set()
            for agent in tasks:
    ...
    def invoke(self, query: str, max_iterations: int=10) -> RAGAgentResponse:
        ...
        final_answer_tool = LLMTool(
            func=final_answer,
            name="final_answer",
            description="Type: `final answer tool`: Call this tool to finalize the answer.",
            args=FinalAnswerArgsSchema
        )

        tools_dict = {"final_answer": final_answer_tool}
        for retriever in selected_retrievers:
            retriever_tool = LLMTool(
                func=retriever.retrieve,
                name=retriever.name,
                description=f"Type: `retriever tool`\n{retriever.description}",
                args=generate_args_schema_from_method(retriever.retrieve)
            )
        ...
        for task in tasks:
            task_tool = LLMTool(
                ...
                args=generate_args_schema_from_method(task.execute)
            )
            tools_dict[task.name] = task_tool

        def call_tool(tool_name, args):
            logger.debug(f"Tool `{tool_name}` Called with args: {args}")
            return tools_dict[tool_name].func(**args)

        tool_metadata = format_tool_metadata(tools_dict)
        dataset_metadata = format_datasets(datasets)

        messages = [
            BaseMessage(role="system", content=CONTROLLER_PROMPT + f"\nAvailable tools:\n{tool_metadata}\n\nRelevant datasets:\n{dataset_metadata}\n"),
            BaseMessage(role="user", content=query)
        ]
        ...
        for i in range(max_iterations):
            try:
                ...
                messages.append(BaseMessage(role="user", content=f"Error: {error}\nOriginal User query: {query}"))
                continue
            ...
            messages.append(tool_call_msg)
            messages.append(BaseMessage(role="user", content=f"Tool Output: {tool_output}\nOriginal User query: {query}"))

    def _select_tasks(self, query):
        task_list = [{"name": task.name, "description": task.description} for task in self.tasks]
        messages = [
            BaseMessage(role="system", content=TASK_SELECTION_PROMPT),
    ...
    def _select_relevant_data(self, query):
        ...
        messages = [
            BaseMessage(role="system", content=DATA_SOURCE_SELECTION_PROMPT),
